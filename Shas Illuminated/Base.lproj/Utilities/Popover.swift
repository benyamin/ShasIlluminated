...
//  Popover
//
//  Created by COPopover on 8/16/15.
...
private extension Popover {
  ...
  func create() {
    ...
    if outerSideEdge > 0 {
      frame.origin.x -= (outerSideEdge + sideEdge) - 10
    } else {
      if frame.minX < 0 {
        frame.origin.x += abs(frame.minX) + sideEdge - 10
      }
    }
    self.frame = frame
    ...
    let trinagleImageview = UIImageView(frame: CGRect(x: arrowPoint.x-30, y:  140, width: 60, height: 20))
    trinagleImageview.image =  UIImage(named: "Meshulash_80X40.png")
